...
function removeLastOccurrences(str, value) {
  ...
  const rValue = [...value].reverse().join('');
  return [...rStr.replace(rValue, '')].reverse().join('');
}
...
function sumOfCodes(str) {
  if (!str) return 0;
  return [...str].reduce((sumCodes, char) => sumCodes + char.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  const normalizedStr = str.replaceAll(/[^a-zA-Zа-яА-Яеё]/gi, '');
  const reverseStr = [...normalizedStr].reverse().join('');
  return normalizedStr.toLocaleLowerCase() === reverseStr.toLocaleLowerCase();
}
...
function reverseWords(str) {
  ...
  const rWords = words.map((word) => [...word].reverse().join(''));
  return rWords.join(' ');
}
...
function encodeToRot13(str) {
  ...
  return [...str].map((c) => output[input.indexOf(c)]).join('');
}
